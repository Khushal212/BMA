...
class AppDatabase extends _$AppDatabase {
  ...
  Future<void> updateItem(Item item) => update(items).replace(item);
  ...
  Future<List<Invoice>> getCustomerInvoices(String customerId) =>
      (select(invoices)
            ...
            ..orderBy([(i) => OrderingTerm(
                expression: i.invoiceDate, mode: OrderingMode.desc)]))
          .get();
  ...
  Future<double> getCustomerOutstanding(String customerId) async {
    ...
    final payRows = await (select(payments)
        ...
        .get();
    final totalBalance =
        invRows.fold<double>(0.0, (s, inv) => s + inv.balanceAmount);
    final totalPaid =
