...
class ShopProfileScreen extends StatefulWidget {
  // Called after a successful save — used to refresh ShopProfileProvider
  final VoidCallback? onSaved;

  const ShopProfileScreen({Key? key, this.onSaved}) : super(key: key);
...
class _ShopProfileScreenState extends State<ShopProfileScreen> {
  ...
  Future<void> _loadProfile() async {
    ...
    final useCustom = await db.getSetting('shop_use_custom_qr');
    if (mounted) {
      setState(() {
  ...
  Future<void> _pickQrImage() async {
    ...
    try {
      final picked = await picker.pickImage(
          source: source, maxWidth: 400, maxHeight: 400, imageQuality: 80);
      if (picked == null) return;
      final bytes     = await File(picked.path).readAsBytes();
      final base64Str = base64Encode(bytes);
      setState(() { _customQrBase64 = base64Str; _useCustomQr = true; });
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('QR image uploaded ✓'),
  ...
  Future<void> _removeCustomQr() async {
    setState(() { _customQrBase64 = null; _useCustomQr = false; });
    final db = context.read<AppDatabase>();
  ...
  Future<void> _save() async {
    ...
    if (_customQrBase64 != null && _customQrBase64!.isNotEmpty) {
      await db.saveSetting('shop_custom_qr', _customQrBase64!);
    }
    await db.saveSetting('shop_use_custom_qr', _useCustomQr.toString());
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Shop profile saved ✓'),
            backgroundColor: Colors.green));
      // Notify caller (MainNavigation) to refresh the gate
      widget.onSaved?.call();
    }
  }
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Row(children: [
                        ...
                        const Expanded(
                          child: Text(
                            'Shop name, address and phone are required to use the app.',
                    ...
                    _sectionTitle('Business Information', Icons.store),
                    ...
                    _sectionTitle('Tax & Payment', Icons.receipt_long),
                    const SizedBox(height: 12),
                    ...
                    TextFormField(
                      ...
                      decoration: const InputDecoration(
                        ...
                        helperText: 'Used to auto-generate QR on invoices',
                      ),
                    ...
                    // QR Section
                    _sectionTitle('Payment QR Code', Icons.qr_code_2),
                    ...
                    Card(
                      child: Padding(
                        ...
                        child: Column(children: [
                          Row(children: [
                            Expanded(child: _QrOptionTile(
                              icon: Icons.auto_awesome,
                              title: 'Auto-Generate QR',
                              subtitle: 'From your UPI ID above',
                              selected: !_useCustomQr,
                              color: AppColors.navy,
                              onTap: () => setState(() => _useCustomQr = false),
                            )),
                            const SizedBox(width: 8),
                            Expanded(child: _QrOptionTile(
                              ...
                              selected: _useCustomQr,
                              color: Colors.orange,
                              onTap: () => setState(() => _useCustomQr = true),
                            )),
                          ]),
                          const SizedBox(height: 16),
                          if (!_useCustomQr) ...[
                              ...
                              GestureDetector(
                                ...
                                child: Container(
                                  ...
                                  child: Column(children: [
                                    ...
                                    const SizedBox(height: 8),
                                    const Text('Tap to upload your QR code',
                                        style: TextStyle(
                                            fontWeight: FontWeight.bold,
                                            fontSize: 14)),
